...
def nueva_moto():
    ...
    if request.method == 'POST':
        ...
        try:
            conexion = mysql.connector.connect(**DB_CONFIG)
            cursor = conexion.cursor()
            sql = "INSERT INTO motocicletas (id_usuario, marca, modelo, anio, cilindrada, kilometraje_actual, foto_url) VALUES (%s, %s, %s, %s, %s, %s, %s)"
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if kilometraje_servicio > moto['kilometraje_actual']:
                cursor.execute("UPDATE motocicletas SET kilometraje_actual = %s WHERE id_moto = %s", (kilometraje_servicio, id_moto))
            
            conexion.commit()
            
            # Notificación enviada al bot de Telegram
            enviar_telegram(f"✅ Mantenimiento registrado: {tipo_servicio} en {moto['marca']} {moto['modelo']}")
